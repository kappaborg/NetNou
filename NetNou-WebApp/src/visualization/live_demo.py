"""Real-time visualization of face analysis with demographic data."""
...
# Make sure we can import from our module structure
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))

try:
    from src.core.nn.simple_nn import SimpleNN
    from src.config import Config
except ImportError:
    print("Error: Could not import required modules.")
    print("Make sure you're running from the NetNou directory.")
    sys.exit(1)
...
config = {
    'analyze_every': Config.ANALYZE_EVERY_N_FRAMES,
...
def run_live_demo(camera_index=0):
    """Run the optimized live demographics analysis demo."""
    global config
    ...
    while True:
        ...
        if frame_count % config['analyze_every'] == 0 or need_detection or not trackers or (frame_count - last_analysis_frame) > Config.MAX_TRACKING_FRAMES:
        ...
        if config['show_fps']:
            ...
            cv2.putText(display_frame, f"Detector: {Config.FACE_DETECTION_BACKEND}", (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        ...
        if key == ord('q'):
            ...
            config['analyze_every'] = Config.ANALYZE_EVERY_N_FRAMES
            config['resize_factor'] = Config.FACE_RESIZE_FACTOR
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Live Demographics Analysis Demo')
    parser.add_argument('--webcam', type=int, default=0, help='Webcam index (default: 0)')
    
    args = parser.parse_args()
    run_live_demo(args.webcam)
